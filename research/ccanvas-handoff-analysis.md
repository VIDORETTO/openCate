# Handoff — Análise profunda do Cate × ccanvas e plano de absorção
...
descreve o estado atual do Cate, resume o que já foi comparado com o
...
O projeto é um fork/produto (branch `product/agent-canvas`) sobre o **Cate**
...
completa de análise comparativa (Cate vs Paseo vs Claude Squad vs Nimbalyst vs
...
(`research/browser-agent-architecture.md`) e Cate Cloud (`plan.md`).
...
- Já existentes no Cate (não reimplementar):
...
1. **Manter o Cate como base.** O ccanvas é um protótipo com UX muito boa, mas
...
2. **NÃO adotar** do ccanvas: scraping-heurístico de status (substituir pelos
   hooks/FSM do Cate), persistência em localStorage, troca de Electron por
   Tauri (Electron é justificado por webview/browser automation e isolamento
   de extensões), duplicação de coisas que o Cate já tem.
3. **Candidatos a portar** (prioridade preliminar):
   - P1 `flow engine` — logic arrows com condições (finish/success/failure/
     ...
     turn-end do barramento de hooks do Cate (mais confiável que o tail do
   ...
   - P1 `checkpoints` — `git stash create` + pin em `refs/cate/cp/<id>`,
     ...
     label/timestamp/branch. A capability de git já existe no Cate
   ...
   - P2 `transcript widget` — visão limpa da conversa da sessão (texto +
     ...
     todas as fontes que o Cate já coleta via `transcriptPath`/`sessionFile`.
   ...
   - P3 `camada de desenho` (freehand, setas com âncoras/curvas, frames,
     ...
     estrutural; avaliar se cabe no modelo de canvas atual do Cate.
...
1. O ccanvas tem alguma característica que muda a estratégia de produto do
   Cate (não apenas de implementação)? Por quê?
2. O flow engine deve ser um recurso de **usuário** (usuário desenha setas)
   ou um contrato de **API/agent** (Cate CLI/agente define edges)? O ccanvas
...
3. Existe risco de incompatibilidade do `on success/failure` com agentes que
   ...
   agente (a resposta do Cate parece ser: hooks `session.idle` + transcript).
...
- Saída esperada (in Portuguese, em arquivo novo em `research/` ou no
  relatório final):
  1. Inspeção do estado atual do Cate no branch `product/agent-canvas`
  ...
  4. Plano priorizado de implementação (fatias verticais, pontos de
     integração com arquivos/linhas do Cate, riscos, estimativa relativa).
  5. Veredito: continuar no Cate e portar; ou mudar alguma frente (e por
     quê).
