...
function DefaultModelSection({ models }: { models: Array<{ provider: string; model: string; label?: string }> }) {
  const [current, setCurrent] = useState<CateAgentModelRef | null>(() => loadDefaultModel())
  ...
  const handlePick = useCallback((m: { provider: string; model: string } | null) => {
    if (!m) {
      ...
      const next: CateAgentModelRef = { provider: m.provider, model: m.model }
...
export function ModelPrefRow({
  ...
  sublabel?: string
  models: PickModels
  current: CateAgentModelRef | null
  open: boolean
