...
// we fire `onChange` so CodingManager can push the update into open workspaces.
//
...
import { sharedAuthPath } from './codingDir'
 ...
 *  workspace's cate-agent dir by agentDir). */
function authJsonPath(): string {
  return sharedAuthPath()
}

async function readAuthJson(): Promise<AuthStorageData> {
  return ((await readCodingConfigFile(authJsonPath())) ?? {}) as AuthStorageData
}

// Fired after every successful write so CodingManager can mirror the shared
// auth.json into open workspaces' cate-agent dirs.
let onAuthChange: (() => void) | null = null

async function updateAuthJson(update: (current: AuthStorageData) => AuthStorageData): Promise<void> {
  await updateCodingConfigFile(authJsonPath(), (current) => update(current as AuthStorageData))
...
export class AuthManager {
   ...
   *  CodingManager to push the shared auth.json into open workspaces. */
  ...
  async listAvailableModels(): Promise<CodingModelDescriptor[]> {
    ...
    const out: CodingModelDescriptor[] = []
  ...
  // the runtime (pi, local or remote — see CodingManager), never the desktop
